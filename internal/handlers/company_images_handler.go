...
func UploadCompanyImageHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "file feld 'image' oder 'file' fehlt", http.StatusBadRequest)
		return
	}
	defer file.Close()
	...
	result, err := repository.UploadCompanyImage(companyID, header.Filename, contentType, fileData)
	if err != nil {
		http.Error(w, fmt.Sprintf("image upload fehlgeschlagen: %v", err), http.StatusBadRequest)
		return
	}
